/**************************************************************************************************************
   ...
   15/05/2026   Helen Outram     4. Added the following tables:
                                               ...
                                               Also changed updated the existing tables to DQA_ instead of DQ_
...
comment on table dqa_dwh_tables is 'Sensitivity Marking: Official. This table is used as part of the DWH Data Quality and Assurance packages. It is used as the main lookup table for the data assurance procedures to run against. One line per table with associated data needed for various lookups';

comment on column dqa_dwh_tables.dwh_ref is 'DWH designated reference - source and target tables are given the same reference. Sensitivity Marking: Official';
comment on column dqa_dwh_tables.schema_name is 'The schema name of the table. Sensitivity Marking: Official';
comment on column dqa_dwh_tables.table_name is 'The table name of the table. Sensitivity Marking: Official';
comment on column dqa_dwh_tables.connection_string is 'The full connection string of the table for use in the PL/SQL package. Sensitivity Marking: Official';
comment on column dqa_dwh_tables.table_type is 'The table type, either source or target. Sensitivity Marking: Official';
comment on column dqa_dwh_tables.unique_row_identifier is 'The column or combination of columns that bring back a distinct row where possible. Sensitivity Marking: Official';
comment on column dqa_dwh_tables.ref_ods_table_name is 'Used only for the source tables, this gives the corresponding ODS table name. For lookup use in the PL/SQL package. Sensitivity Marking: Official';
